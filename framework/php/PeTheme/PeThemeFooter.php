...
class PeThemeFooter {
	...
	public $layout = 
		array(
			  "default" => array("span3","span6","span3"),
	...
	public function widgets() {
		...
		$this->master->sidebar->show("footer",!(is_single() || is_page()));
	...
	public function dynamic_sidebar_params_filter($params) {
		$p =& $params[0];
		$classes = $this->layout[$this->master->options->get("footerLayout")];
		//$classes = $this->layout["3col"];
		$last = count($classes);
		//if ($p["name"] == "footer") {
		$class = @$classes[$this->count++ % $last];
		if ($this->count == $last) $class .= " last";
		$p["before_widget"] = "<div class=\"$class footer\">".$p["before_widget"];
		$p["after_widget"] .= '</div>';
		//}
		return $params;
	}
