...
class PeThemeProject {
	...
	public function filter($sep = "",$aclass="label") {
		...
		if (is_array($terms) && count($terms) > 0) {
			$output = sprintf('<a class="%s active" data-category="" href="#">%s</a>%s',$aclass,__pe("All"),"$sep\n");
			foreach ($terms as $term) {
				if ($tags && !isset($tags[$term->slug])) continue;
				$output .= sprintf('<a class="%s" data-category="%s" href="#">%s</a>%s',$aclass,$term->slug,$term->name,"$sep\n");
			}
			print $output;
		}
	}
