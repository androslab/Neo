		...
		<div class="span12">
			<form action="<?php $comments->action(); ?>" method="post" id="commentform" class="form-horizontal">
				...
				<div class="form-submit">
					<div class="controls">
						<button class="btn <?php echo apply_filters("pe_theme_comment_submit_class","btn-success"); ?>" type="submit">Post Comment</button>
						<?php $comments->fields(); ?>
					</div>
				</div>
			</form>
...
<?php
	...
	public function intro_gallery($id,$w,$h,$name,$cols = 4,$class = "span3") {
		$loop =& $this->master->gallery->getSliderLoop($id,$w,$h,$cols,$class);
		if (!$loop) return;
		if (!$this->custom($loop,"intro-gallery",$name)) {
			$this->{"intro_gallery_$name"}($loop);
		}
	}

	public function intro_gallery_thumbnails($loop) {
...
<div class="<?php echo $loop->main->class; ?>">
	<a 
		...
		>
		<?php echo $t->image->resizedImg($item->img,$width,$height); ?>
	</a>
</div>
...
<?php
	...
	public function intro_gallery_fullscreen($loop) {
		$width =& $loop->main->width;
		$height =& $loop->main->height;
		$id =& $loop->main->id;
		$t =& peTheme();
...
<?php
	...
	public function gallery_cover($w,$h) {
		$loop = false;
		...
		if ($t->content->type() != "gallery") {
			...
			$gallery = $t->content->meta()->gallery;
			$id = $gallery->id;
			$type = $gallery->type;
...
<div class="portfolioItem galleryCover">
	...
	<?php else: ?>		
	<a class="<?php echo $overClass; ?>" href="<?php $t->content->link() ?>">
	<?php endif; ?>
