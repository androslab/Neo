...
class PeThemeImporter extends WP_Import {
	...
	public function import_start($file) {
		$this->updateStats(__pe("Parsing import file"));
		parent::import_start($file);
	}

	public function process_categories() {
		$this->updateStats(__pe("Importing categories"));
		parent::process_categories();
	}

	public function process_terms() {
		$this->updateStats(__pe("Importing terms"));
		parent::process_terms();
	}

	public function process_posts() {
		$this->updateStats(__pe("Importing posts"));
		parent::process_posts();
	}

	public function backfill_parents() {
		$this->updateStats(__pe("Fixing orphans"));
	...
	public function backfill_attachment_urls() {
		$this->updateStats(__pe("Fixing urls"),true);
		...
		// fix post metas
		$metaKey = "pe_theme_".(PE_THEME_NAME);
	...
	public function remap_featured_images() {
		$this->updateStats(__pe("Remapping images"),true);
		global $wpdb;
		$mt = $wpdb->postmeta;
		$pt = $wpdb->posts;
	...
	public function import_end() {
		$this->updateStats(__pe("Final cleanup"),true);
		parent::import_end();
	}
