...
class PeThemeAdmin {
	...
	public function admin_menu() {
		...
		$checkEvery = intval($this->options->updateCheck);
		$checkEvery = 0;
		
		// debugging
		//delete_transient("pe_theme_update_check");
		//delete_transient("pe_theme_update");

		if ($checkEvery > 0 && $this->options->updateUsername && $this->options->updateAPIKey && current_user_can("update_themes")) {
			// only check once each $checkEvery amount of seconds
			if (get_transient("pe_theme_update") || !get_transient("pe_theme_update_check")) {
				$this->master->update->check();
				set_transient("pe_theme_update_check","checked",$checkEvery);
			}
			$this->add_non_menu_page("pe_theme_update",array(&$this,"theme_update"));
		} else {
			delete_transient("pe_theme_update_check");
			delete_transient("pe_theme_update");
		}
	...
	public function admin_init() {
		...
		add_editor_style("framework/css/editor.css");
