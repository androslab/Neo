...
(function ($) {
	...
	var template = '<tr class="col-heading">\
        <td class="col-title" colspan="3">%0<input name="%1[]" type="hidden" value="%0"></td>\
        <td class="col-edit">\
	...
	function PeFieldSidebars(target, conf) {
		...
		var title = target.find("input[type=text]");
		...
		function add() {
			var value = auto ? auto+(count()+1) : title.val().replace(/["<>\\'\n\r\t?&]/g,"");
			title.val("");
			if (addValue(value)) {
		...
		function addValue(value) {
			...
			if (!value || value === "" || sidebars[value]) {
				return false;
			}
			
			tbody.append(customTemplate.format(value,name));
			sidebars[value] = true;
			total++;
